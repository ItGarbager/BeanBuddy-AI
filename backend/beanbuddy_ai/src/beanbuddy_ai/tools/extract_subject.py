...
class ExtractSubjectConfig(FunctionBaseConfig, name="extract_subject"):
    ...
    """
    # 1. 通义千问API密钥（优先级：配置内设置 > 环境变量）
    api_key: str = Field(
        default="",
        description="阿里云DashScope API密钥（必填），用于调用qwen-image-edit模型。"
                    "也可通过环境变量 'DASHSCOPE_API_KEY' 配置。"
    )

    # 2. 图片处理指令（可自定义，默认满足Q版拼豆主体提取需求）
    text_instruction: str = Field(
        default="1. 自动识别图片中的核心形象主体（如人物、动物、物体），排除背景杂物；"
                "2. 完整保留主体轮廓细节（包括毛发、透明材质、细小装饰的边缘），彻底移除所有背景元素；"
                "3. 输出严格符合PNG-24格式标准的图片，必须包含alpha透明通道（背景像素透明度100%），"
...
async def extract_subject_function(
    ...
    dashscope_api_key = config.api_key or os.getenv("DASHSCOPE_API_KEY")
    if not dashscope_api_key:
        error_msg = (
            "ExtractSubject工具初始化失败：未找到DashScope API密钥！\n"
            "解决方案：\n"
            "1. 在config.yml的 'extract_subject' 函数配置中添加 'api_key' 字段；\n"
    ...
    # 2. 核心工具函数（异步实现）
    # --------------------------
    async def _extract_subject(image_url: str) -> str:
        """
        工具核心逻辑（nemo-agent实际调用的函数）
        Args:
            image_url: 原始图片的公网HTTP/HTTPS URL（必须可公开访问）
        Returns:
        ...
        if not image_url.startswith(("http://", "https://")):
            error_msg = f"无效图片URL：{image_url}（必须是公网HTTP/HTTPS链接）"
            logger.error(error_msg)
            return error_msg
        ...
        # 捕获API调用异常（网络错误、超时等）
            ...
            error_msg = f"API调用超时（超过{api_timeout}秒），请检查图片URL有效性或延长超时时间"
            logger.exception(error_msg)
            return error_msg
        except Exception as e:
            error_msg = f"API调用异常：{str(e)}（请检查API密钥有效性或网络连接）"
            logger.exception(error_msg)
            return error_msg
        ...
        try:
            ...
            if not processed_url:
                error_msg = "API响应解析失败：未找到处理后的图片URL（响应结构异常）"
